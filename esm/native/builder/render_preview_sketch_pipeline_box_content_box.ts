...
export function applyBoxVolumeSketchPlacementPreview(ctx: SketchPlacementPreviewContext): boolean {
  ...
  const thickness = Math.max(SKETCH_BOX_PREVIEW_CORE_POLICY.minScaleM, ctx.woodThick);
  ...
  const frontOverlay = ctx.readFrontOverlay(
    ...
    ctx.w,
    boxH,
    Math.max(SKETCH_BOX_BOX_PREVIEW_POLICY.boxFillThicknessMinM, Math.min(ctx.d, thickness))
  );
  ...
  const backFillT = Math.max(SKETCH_BOX_BOX_PREVIEW_POLICY.boxFillThicknessMinM, Math.min(ctx.d, thickness));
  ...
  const setCenterMarker = () => {
    ...
    const markerT = Math.max(
      SKETCH_BOX_BOX_PREVIEW_POLICY.boxCenterMarkerThicknessMinM,
      Math.min(thickness, SKETCH_BOX_BOX_PREVIEW_POLICY.boxCenterMarkerThicknessMaxM)
    );
