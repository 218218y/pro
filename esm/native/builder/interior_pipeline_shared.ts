...
export function readNumber(value: unknown, defaultValue: number): number {
  return readBuilderDraftGeometryNumber(value, defaultValue);
}
...
export function buildSketchExtrasArgs(
  ...
  const sketchExtras = normalizeBuilderDraftSketchExtrasGeometry(config.sketchExtras);
  if (!sketchExtras) {
  ...
  return {
    App,
    roomArchitecturePlan,
    ...(input.THREE !== undefined ? { THREE: input.THREE } : {}),
    cfgSnapshot,
    ...(input.wardrobeGroup !== undefined ? { wardrobeGroup: input.wardrobeGroup } : {}),
    createBoard: input.createBoard ?? null,
    createRod: input.createRod ?? null,
    ...(input.currentShelfMat !== undefined ? { currentShelfMat: input.currentShelfMat } : {}),
    ...(input.currentBraceShelfMat !== undefined ? { currentBraceShelfMat: input.currentBraceShelfMat } : {}),
    ...(input.bodyMat !== undefined ? { bodyMat: input.bodyMat } : {}),
    ...(input.whiteMat !== undefined ? { whiteMat: input.whiteMat } : {}),
    ...
    ...(input.hingedDoorPivotMap !== undefined ? { hingedDoorPivotMap: input.hingedDoorPivotMap } : {}),
    ...
    ...(input.createDoorVisual !== undefined ? { createDoorVisual: input.createDoorVisual } : {}),
    doorStyle,
    ...(input.createInternalDrawerBox !== undefined
      ? { createInternalDrawerBox: input.createInternalDrawerBox }
      : {}),
    addOutlines: input.addOutlines ?? null,
    sketchMode: input.sketchMode === true,
    ...(input.showContentsEnabled !== undefined ? { showContentsEnabled: input.showContentsEnabled } : {}),
    isGroovesEnabled: input.isGroovesEnabled === true,
    isInternalDrawersEnabled: input.isInternalDrawersEnabled === true,
    addFoldedClothes: input.addFoldedClothes ?? null,
    sketchExtras,
  };
}
