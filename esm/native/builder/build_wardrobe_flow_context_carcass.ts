...
export function computeBuildWardrobeSplitLineY(args: {
  ...
  const gridStep = internalTotalHeight / CARCASS_INTERIOR_GRID_POLICY.divisions;
  return startY + woodThick + CARCASS_INTERIOR_GRID_POLICY.drawerSplitLineIndex * gridStep;
}
...
export function resolveBuildWardrobeCarcassMetrics(args: {
  ...
  const carcassArgs: Parameters<typeof applyCarcassAndGetCabinetMetrics>[0] = {
    ...
    baseLegPlatformSideOverhangCm: plan.baseLegPlatformSideOverhangCm,
    baseLegPlatformFrontOverhangCm: plan.baseLegPlatformFrontOverhangCm,
    basePlinthHeightCm: plan.basePlinthHeightCm,
    baseLegHeightCm: plan.baseLegHeightCm,
    baseLegWidthCm: plan.baseLegWidthCm,
    baseLegTopPlatformOnly: plan.baseLegTopPlatformOnly,
