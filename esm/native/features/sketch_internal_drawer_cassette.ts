...
export const SKETCH_INTERNAL_DRAWER_CASSETTE_PART_SUFFIX = '_cassette';

export function createSketchInternalDrawerCassettePartId(stackPartId: unknown): string {
  const raw =
    typeof stackPartId === 'string' || typeof stackPartId === 'number' ? String(stackPartId).trim() : '';
  return raw
    ? `${raw}${SKETCH_INTERNAL_DRAWER_CASSETTE_PART_SUFFIX}`
    : `internal_drawer${SKETCH_INTERNAL_DRAWER_CASSETTE_PART_SUFFIX}`;
}

export function isSketchInternalDrawerCassettePartId(partId: unknown): boolean {
  const raw = typeof partId === 'string' ? partId.trim() : '';
  return !!raw && raw.endsWith(SKETCH_INTERNAL_DRAWER_CASSETTE_PART_SUFFIX);
}
