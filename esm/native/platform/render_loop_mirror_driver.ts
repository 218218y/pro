...
function resolvePlanarUpdatesPerFrame(A: AppContainer, motionActive: boolean): number {
  const raw = motionActive
    ? readConfigNumberLooseFromApp(A, 'MIRROR_REFLECTOR_MOVE_MAX_UPDATES_PER_FRAME', 8)
...
export function createRenderLoopMirrorDriver(
  ...
  function updateMirrorCube(): void {
    ...
    const cubeMirrorMode = readConfigLooseScalarFromApp(A, 'MIRROR_REFLECTOR_ENABLED', true) === false;
