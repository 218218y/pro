...
import type { ProjectLoadTerminalResult } from '../runtime/project_load_action_result.js';

export function createProjectFileLoadHandler(deps: ProjectIoOwnerDeps) {
  const { App, showToast } = deps;
  return async function handleFileLoad(eventOrFile: unknown): Promise<ProjectLoadTerminalResult> {
