...
function requireScript(name) {
  if (!Object.prototype.hasOwnProperty.call(scripts, name)) errors.push(`package.json: missing script ${name}`);
...
for (const script of requiredGuardScripts) requireNeedle('check:refactor-guardrails', guardrailCommand, `npm run ${script}`);
