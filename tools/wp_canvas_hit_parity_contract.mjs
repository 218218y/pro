...
requireMatch(files.owner, owner, /export function mergeCanvasPickingHitIdentityUserData\(/, 'hit identity owner must expose shared hover/click metadata merger');
requireMatch(files.owner, owner, /const merged: UnknownRecord = \{\s*\.\.\.\(resolved \|\| \{\}\),\s*\.\.\.\(hit \|\| \{\}\),\s*\};/s, 'shared metadata merger must preserve surface hit metadata over resolved parent defaults');
...
requireMatch(files.clickScanObjects, clickScanObjects, /mergeCanvasPickingHitIdentityUserData/, 'click scan must use the shared canonical metadata merger');
...
requireMatch(files.clickScanObjects, clickScanObjects, /const resolvedDoorId =\s*\n\s*typeof mergedUserData\?\.doorId === 'string'/, 'click scan must prefer canonical resolved door id from merged metadata when available');

const hoverScan = read(files.hoverScan);
requireMatch(files.hoverScan, hoverScan, /mergeCanvasPickingHitIdentityUserData/, 'hover scan must use the shared canonical metadata merger');
...
requireMatch(files.hoverClickRuntimeTest, hoverClickRuntimeTest, /hover and click preserve the same child-surface door identity/, 'runtime test must prove hover/click child-surface parity');
