...
export function buildCommonRuntimeConfigEnvelope(profile) {
  return {
    flags: { ...profile.commonFlags },
    config: {
      cacheBudgetMb: 128,
      cacheMaxItems: 2000,
      ...
      supabaseCloudSync: { ...profile.supabase },
      ...profile.commonConfig,
    },
  };
}

export function buildRuntimeConfigVariantOverlay(profile, variantName) {
  ...
  return {
    flags: { ...variant.extraFlags },
    config: {
      siteVariant: variant.siteVariant,
      ...(variant.siteVariant === 'site2' ? { site2EnabledTabs: variant.site2EnabledTabs } : {}),
...
export function mergeRuntimeConfigEnvelopes(base, overlay) {
  const baseConfig = isRecord(base?.config) ? base.config : {};
  const overlayConfig = isRecord(overlay?.config) ? overlay.config : {};
  const config = { ...baseConfig, ...overlayConfig };
  for (const key of ['orderPdf', 'supabaseCloudSync']) {
    if (isRecord(baseConfig[key]) || isRecord(overlayConfig[key])) {
      config[key] = {
        ...(isRecord(baseConfig[key]) ? baseConfig[key] : {}),
        ...(isRecord(overlayConfig[key]) ? overlayConfig[key] : {}),
      };
    }
  }
  return {
    flags: {
      ...(isRecord(base?.flags) ? base.flags : {}),
      ...(isRecord(overlay?.flags) ? overlay.flags : {}),
    },
    config,
  };
}

export function buildRuntimeConfigEnvelope(profile, variantName) {
  return mergeRuntimeConfigEnvelopes(
    buildCommonRuntimeConfigEnvelope(profile),
    buildRuntimeConfigVariantOverlay(profile, variantName)
  );
}

export function buildRuntimeConfigSource(profile, variantName, options = {}) {
  const generator = asString(options.generator, 'tools/wp_release_site.js');
  const targetDescription = asString(options.targetDescription, 'generated release copy');
  const envelope = buildRuntimeConfigEnvelope(profile, variantName, options);

  return (
    `// Auto-generated by ${generator} from ${profile.profileRelDir}/site.profile.mjs.\n` +
    `// Do not edit this ${targetDescription} directly; edit the site profile instead.\n` +
    `export default ${safeJson(envelope)};\n`
  );
}

export function buildRootRuntimeConfigSource(profile) {
  if (profile.id !== 'bargig') {
    throw new Error('[WP Runtime Config] Root runtime config must be generated from the Bargig profile');
  }
  const commonEnvelope = buildCommonRuntimeConfigEnvelope(profile);
  const overlays = {
    main: buildRuntimeConfigVariantOverlay(profile, 'main'),
    site2: buildRuntimeConfigVariantOverlay(profile, 'site2'),
  };
  return (
    `// Auto-generated by tools/wp_generate_runtime_config.mjs from ${profile.profileRelDir}/site.profile.mjs.\n` +
    '// Do not edit this root runtime config directly; edit the site profile instead.\n' +
    `export const runtimeConfigOverlays = ${safeJson(overlays)};\n` +
    `export default ${safeJson(commonEnvelope)};\n`
  );
}
