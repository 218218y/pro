...
export function buildRuntimeConfigEnvelope(profile, variantName, options = {}) {
  ...
  const includeVariantIdentity = options.includeVariantIdentity !== false;
  ...
  if (includeVariantIdentity) config.siteVariant = variant.siteVariant;
  if (includeVariantIdentity && variant.siteVariant === 'site2') {
    config.site2EnabledTabs = variant.site2EnabledTabs;
  }

  return { flags: { ...variant.extraFlags }, config };
}
...
export function buildRootRuntimeConfigSource(profile) {
  ...
  return buildRuntimeConfigSource(profile, 'main', {
    generator: 'tools/wp_generate_runtime_config.mjs',
    targetDescription: 'root runtime config',
