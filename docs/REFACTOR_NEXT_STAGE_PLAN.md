...
The latest audit hygiene slice is complete: React-owned `Suspense fallback` props are categorized as `framework-default`, normal local default paths were renamed or documented without `fallback` terminology, and adapter/default wording was reviewed against the live code. The generated Stage 42 inventory is now reduced to **11** occurrences across **6** files. The important queues are clean: `runtime-default: 0`, `legacy-runtime-risk: 0`, and `unknown: 0`. Remaining occurrences are limited to one browser-adapter compatibility mirror, one project migration boundary, test fixtures, and framework-owned default props.

Future legacy/default work should not rename terms mechanically. Start from the current 11-item inventory and treat the remaining non-fixture items as decision work: retire `App.browser.userAgent` only when its compatibility mirror is no longer a public/browser-adapter contract, and remove the project payload migration marker only if the supported persisted-schema horizon is narrowed deliberately. Framework-default entries should remain excluded from runtime-risk burn-down unless they stop being framework API names.
