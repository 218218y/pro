...
2. **Configuration and edit-state mutation — next.** Migrate `config_compounds_*` and `edit_state_*` paths where a swallowed setter, reset, or synchronization failure can leave partially applied user state. Keep normalization-only fallbacks separate from mutation failures.
...
The immediate next slice is item 2: configuration and edit-state mutation, beginning with config compound writes that can leave partially applied durable user state.
