...
- Error-observability modernization is active and capability-based. The canonical Oxc-based silent-catch policy distinguishes 725 statement-free catches from the 77 truly bare catches and owns exact current-state layer ratchets for both. History operations, restore-state reads, timer cleanup, boot command finalizers, boot seed config/action/write fallbacks, the Autosave snapshot/storage/scheduling pipeline, and the camera access/motion pipeline now report nonfatal owner failures while preserving fail-soft behavior. Autosave continues from a rejected Project Capture result to the canonical History snapshot fallback, while camera motion continues through vector-copy, manual-interpolation, and timer scheduling fallbacks instead of silently abandoning the user action.
