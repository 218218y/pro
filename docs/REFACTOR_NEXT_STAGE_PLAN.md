...
- Error-observability modernization is active and capability-based. The canonical Oxc-based silent-catch policy distinguishes 739 statement-free catches from the 87 truly bare catches and owns exact current-state layer ratchets for both. History operations, restore-state reads, timer cleanup, boot command finalizers, boot seed config/action/write fallbacks, and the Autosave snapshot/storage/scheduling pipeline now report nonfatal owner failures while preserving fail-soft behavior. Autosave also continues from a rejected Project Capture result to the canonical History snapshot fallback instead of abandoning the save.
