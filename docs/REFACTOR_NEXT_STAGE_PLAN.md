...
The latest audit hygiene slice is complete: React-owned `Suspense fallback` props are categorized as `framework-default`, normal local default paths were renamed or documented without `fallback` terminology, and the generated Stage 42 inventory is now reduced to **19** occurrences across **12** files. The important queues are clean: `runtime-default: 0`, `legacy-runtime-risk: 0`, and `unknown: 0`. Remaining occurrences are limited to browser/DOM adapter boundaries, one project migration boundary, test fixtures, and framework-owned default props.

Future legacy/default work should not rename terms mechanically. Start from the current 19-item inventory and review only the remaining non-fixture categories: browser-adapter entries should stay at browser boundaries unless they hide an avoidable live compatibility path; the single project-migration entry should stay only if it is still needed for persisted payload ingress; framework-default entries should remain excluded from runtime-risk burn-down unless they stop being framework API names.
