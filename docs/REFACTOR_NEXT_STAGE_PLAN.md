...
The immediate next slice is item 5: Boot/install and UI state mirrors. Prioritize startup/install writes whose failure leaves a capability absent and UI mirrors whose rejected write leaves visible state stale. Keep optional browser capability detection, hover previews, disposal, shader warmup, and visual cleanup classified as best-effort unless they mutate canonical business state.
