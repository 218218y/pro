...
The immediate next slice is item 3: Cloud Sync persistence and reconciliation. Start with durable collection commits, conflict-journal writes, pull/adoption transitions, and status publication paths where a rejected owner can lose a commit or leave the state machine inconsistent; keep offline/reconnect detection and optional transport cleanup fail-soft.
