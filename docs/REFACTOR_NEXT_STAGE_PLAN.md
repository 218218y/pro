...
4. **Render, measurement, and Canvas commit paths — commit-critical core complete; specialized structural authoring remains.** History batching no longer replays a mutation after canonical execution has started; paint refresh/render occurs only after the config mutation completes; camera pose writes roll back to the prior snapshot if controls/projection update fails; renderer setup fails closed; render-scheduler follow-through reports through the canonical platform rejection path; and committed measurement-mode/render failures publish stable diagnostics. Hover preview, disposal, shader warmup, and optional visual cleanup remain reviewed best-effort. The remaining work in this lane is specialized Canvas structural-authoring writers whose missing/rejected module/config writes can still appear as a no-op.
...
The immediate next slice is the remaining structural-authoring portion of item 4: Canvas module/config writers and specialized click commits whose canonical write can still be rejected or unavailable without a durable result. Complete that family before item 5. Keep hover previews, disposal, shader warmup, and optional visual cleanup classified as best-effort unless they mutate canonical state.
