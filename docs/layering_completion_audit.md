...
- `builder/visuals_contents_shared.ts` resolves library shelf contents only from the explicit build `cfgSnapshot` and hanger visibility only from an explicit build flag; folded-content callers pass the snapshot through preset, custom, sketch, drawer, and corner flows, while hanger callers pass canonical `showHangerEnabled`, with no live `App` config/UI fallback
