...
- `builder/visuals_contents_shared.ts` resolves folded and hanging content visibility, library mode, and door style only from explicit build policies; preset, custom, sketch, drawer, module-loop, rod, and corner callers propagate those policies without live `App` config/UI reads, compatibility overrides, or fallback shape guessing, while hanger visibility remains an explicit build flag
