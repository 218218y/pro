...
- `builder/visuals_contents_shared.ts` resolves library shelf contents only from the explicit build `cfgSnapshot`; folded-content callers pass that snapshot through preset, custom, sketch, drawer, and corner flows with no live `App` config fallback
