...
- `builder/post_build_removed_parts.ts` requires the post-build removed-parts `cfgSnapshot` and reads `removedDoorsMap` only from that snapshot, so visual cleanup cannot fall back to live App config after a build pass
