...
- `builder/handles_config_snapshot.ts` owns required handle map snapshots, while handle apply/refresh also requires the captured outline binding; `handles_mesh.ts` never invokes raw outline ops, and post-build, Chest Mode, material refresh, and UI callers provide one explicit binding per pass
