...
- `kernel/state_api_stack_router.ts` owns stack-aware module/corner ensure and patch materialization; domain `ensureAt`/`patchAt` and corner ensure/patch methods are one-way aliases into `ensureForStack`/`patchForStack`, with no reverse callback lookup or delegate markers
