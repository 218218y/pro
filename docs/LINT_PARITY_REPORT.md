...
| lint JS/parser-removal dry-run | `npm run lint:js:strict`                              | yes       | ESLint profile that excludes TS/TSX and keeps JS/tools/tests/config coverage, including `no-undef`. | strict blocking dry-run with 0 JS warnings; proves TS/TSX can leave `@typescript-eslint/parser` while JS remains linted |
