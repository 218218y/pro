...
| `oxlint`          | `^1.75.0`               | `^1.75.0`    | `^1.75.0`         | `1.78.0`              | `>=1.75.0 <2.0.0`       | Blocking TS/TSX syntax lint gate.                                                 | Allow Oxlint 1.x patch/minor releases while the syntax and type-aware lanes remain at zero diagnostics.                                                  |
...
| `oxc-parser`      | `>=0.143.0 <0.144.0`    | `^0.144.0`   | `^0.144.0`        | `0.144.0`             | `>=0.143.0 <0.144.0`    | Internal AST adapter parser used by production contracts and code-analysis tools. | Allow reviewed 0.143.x patch releases. The separately signed offline fallback may remain on 0.142.x while both lines pass the same AST adapter contract. |
...
- oxc-parser must use approved range >=0.143.0 <0.144.0; found ^0.144.0.
- oxc-parser resolved version 0.144.0 does not satisfy approved policy >=0.143.0 <0.144.0.
