...
| `oxlint`          | `^1.79.0`               | `^1.79.0`    | `^1.79.0`         | `1.81.0`              | `>=1.75.0 <2.0.0`       | Blocking TS/TSX syntax lint gate.                                                 | Allow Oxlint 1.x patch/minor releases while the syntax and type-aware lanes remain at zero diagnostics.                                                                                                                      |
...
| `oxc-parser`      | `^0.148.0`              | `^0.148.0`   | `^0.148.0`        | `0.148.0`             | `>=0.148.0 <0.149.0`    | Internal AST adapter parser used by production contracts and code-analysis tools. | Allow the active Oxc 0.x patch line declared by package.json. Moving to the next Oxc minor updates the reviewed patch window during dependency synchronization, then the AST adapter and offline vendor contracts must pass. |
