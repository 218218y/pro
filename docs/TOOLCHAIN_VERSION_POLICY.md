...
| `oxlint`          | `^1.75.0`               | `^1.75.0`            | `^1.75.0`            | `1.76.0`              | `>=1.75.0 <2.0.0`       | Blocking TS/TSX syntax lint gate.                                                 | Allow Oxlint 1.x patch/minor releases while the syntax and type-aware lanes remain at zero diagnostics.                                |
...
| `oxc-parser`      | `>=0.142.0 <0.143.0`    | `>=0.142.0 <0.143.0` | `>=0.142.0 <0.143.0` | `0.142.0`             | `>=0.142.0 <0.143.0`    | Internal AST adapter parser used by production contracts and code-analysis tools. | Allow reviewed 0.142.x patch releases. The separately signed offline fallback may lag inside the same tested AST compatibility window. |
