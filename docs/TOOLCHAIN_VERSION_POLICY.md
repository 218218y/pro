...
| `oxc-parser`      | `^0.144.0`              | `^0.144.0`   | `^0.144.0`        | `0.144.0`             | `>=0.144.0 <0.145.0`    | Internal AST adapter parser used by production contracts and code-analysis tools. | Allow the active Oxc 0.x patch line declared by package.json. Moving to the next Oxc minor updates the reviewed patch window during dependency synchronization, then the AST adapter and offline vendor contracts must pass. |
