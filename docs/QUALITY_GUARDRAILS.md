...
- Geometry descriptor families promoted to Build IR must have one producer/consumer type contract, stable semantic discriminants, finite/positive numeric invariants, and a runtime matrix for the feature's high-risk combinations before Three.js emission. The main carcass now uses this boundary for both cornice and shell geometry: `carcass_cornice_ir.ts` owns cornice segments, while `carcass_shell_ir.ts` owns shell boards/back panels across normal, stepped, removed-side, and stack-split-prefixed identities. Renderer-side duplicate guards and generic `MutableRecord`/unknown field rediscovery are not part of these boundaries. `lint-architecture/typed-ir:carcass-shell` prevents the shell producer or renderer seam from drifting back to the old record/guard pattern.
