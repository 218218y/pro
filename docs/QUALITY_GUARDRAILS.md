...
- `check:silent-catches` parses production sources with Oxc and owns two exact current-state ratchets by layer: all statement-free catches (including documented best-effort blocks) and the narrower bare-catch subset with no statement or explanation. The current baseline is 617 statement-free catches with **0 bare catches in all production layers**. A third exact ratchet tracks the remaining 353 vague historical comments whose body is only `// ignore` or `// swallow`; these are the remaining classification-closeout debt and cannot increase. Kernel History/Meta follow-through, preset-model boot installation, lifecycle recovery, Sketch Mode commits, runtime Door commands, and Autosave schedule/cancel/flush access are now observable rather than silently dropping required follow-through or publishing stale UI state. Any increase fails immediately; any reduction must update the exact ratchet in the same change so that removed debt cannot return. Functional owners that have completed observability migration are separately locked to zero statement-free catches.
