...
- Targeted Canvas hover geometry keeps container adaptation outside actively changed cores. `canvas_picking_door_layout_alignment.ts` consumes three door-layout read capabilities, while `canvas_picking_cell_dims_free_box_hover.ts` consumes five config/viewport/measurement/post-click/raycast capabilities. Their `*_runtime.ts` adapters are the only modules in those pairs that accept `AppContainer`, are weakly cached on pointer-movement paths, and architecture lint rejects runtime/kernel imports or `AppContainer` leakage back into the cores. Capability-named runtime tests execute the core decisions without an application object.
...
- Browser performance schema 28 binds every generated regression baseline to one complete measurement profile and tracks CLS, LCP, Long Task count/total/p95, render-settle, and INP. Repeated-action drift is enforced only for clean, comparable sequences and only when the late-half code-execution increase is at least 20ms; mixed recovery outcomes are covered by their outcome/transition/recovery contracts, while sub-20ms timer noise remains visible in reports without becoming a percentage-only gate. Dominant Store source share is always reported together with its absolute milliseconds, but the ratio is blocking only when the journey's total Store source time is at least 20ms. Runtime spans dominated by explicit interaction wait remain visible but are not duplicated in the internal runtime-UX budget: full customer-journey budgets and app-owned code-execution budgets remain blocking. Small store/recovery duration overruns also require at least 20ms of material excess. Vite's dynamic React module load is measured separately as `boot.browser.react-module-load`; `boot.browser.setup` starts only after that module settles, while release keeps its static import and remains the product-facing performance gate. INP is collected from Event Timing entries grouped by `interactionId`, estimated as the p98 interaction, and falls back to `first-input` when no interaction id is available. The dev regression profile has a 500ms INP ceiling to absorb dev-server scheduling variance; the fixed product target remains 200ms and the release regression floor remains 200ms. Browser evidence requires usable CLS, LCP, and INP measurement semantics: zero CLS is valid only when the layout-shift observer capability is present, LCP requires at least one observed entry, and INP requires a usable interaction. Missing evidence is `unmeasured`, never a zero-duration pass.
