...
- `check:silent-catches` parses production sources with Oxc and owns two exact current-state ratchets by layer: all statement-free catches (including documented best-effort blocks) and the narrower bare-catch subset with no statement or explanation. The current baseline is 650 statement-free catches, of which 61 are bare; the services layer remains at zero bare catches. Platform boot, UI boot failure rollback, ready-state/autosave convergence, and the Primary Mode DOM mirror are now observable rather than silently accepting stale boot/UI state. Any increase fails immediately; any reduction must update the exact ratchet in the same change so that removed debt cannot return. Functional owners that have completed observability migration are separately locked to zero statement-free catches.
