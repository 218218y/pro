...
- `check:silent-catches` parses production sources with Oxc and owns two exact current-state ratchets by layer: all statement-free catches (including documented best-effort blocks) and the narrower bare-catch subset with no statement or explanation. The current baseline is 750 statement-free catches, of which 87 are bare. Any increase fails immediately; any reduction must update the exact ratchet in the same change so that removed debt cannot return. Functional owners that have completed observability migration are separately locked to zero statement-free catches.
- Protected functional owners now include boot finalizers, the boot-seed colors/flags/runtime pipeline, and the complete history runtime/schedule/shared family. Expand this set only when a capability migration includes focused fail-soft and diagnostic behavior tests; do not add per-stage proof files.
