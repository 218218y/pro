...
1. **Capability DI pilot.** On one actively maintained feature boundary, replace broad `AppContainer`/runtime-access dependency reach with an explicit narrow capability object; expand only if the pilot measurably reduces imports, mocks, and accessors.
2. **Typed geometry/build IR.** Convert geometry descriptor families to discriminated typed plans when those areas are actively changed, enabling deterministic combinatorial invariant tests before Three.js emission.
