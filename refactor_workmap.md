...
- Typed Geometry / Build IR has a completed first family on the main carcass cornice. `carcass_cornice_ir.ts` is the single contract shared by the pure carcass producer and renderer: wave-front, wave-side, and profile segments are discriminated unions with finite/positive geometry invariants and mode/segment consistency. The renderer boundary rejects malformed IR instead of rediscovering fields from generic records, and a high-risk runtime matrix covers classic/wave, removed frame sides, stepped height/depth, and hex footprints before Three.js emission.
...
1. **Typed geometry/build IR expansion on evidence.** The main carcass cornice pilot is complete. Extend the pattern only to an actively changed descriptor family with dynamic-record risk or combinatorial geometry bugs; reuse the same producer → validated IR → renderer boundary rather than introducing a parallel geometry contract.
