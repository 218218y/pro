...
- Contract Engine modernization has a completed first family: all ten Dimension Composition owners now share one declarative manifest and one semantic AST/provenance engine, with runtime identity evidence kept behind the existing CI entry points. Exact export statement ordering, duplicated contract declarations, regex shape checks, and stale body hashes were removed from this family; mutation tests prove harmless reordering passes while owner bypasses, aliases, copied logic, and provenance drift fail.
...
1. **TypeScript/control-plane simplification.** Promote the whole-project strict check to the primary type source of truth, then remove redundant per-layer typecheck modes/scripts only after proving they add no unique coverage.
2. **Capability DI pilot.** On one actively maintained feature boundary, replace broad `AppContainer`/runtime-access dependency reach with an explicit narrow capability object; expand only if the pilot measurably reduces imports, mocks, and accessors.
3. **Typed geometry/build IR.** Convert geometry descriptor families to discriminated typed plans when those areas are actively changed, enabling deterministic combinatorial invariant tests before Three.js emission.
4. **Contract Engine expansion on demand.** The Dimension Composition pilot is complete. Migrate another source/hash-heavy family only when it is actively changed or its implementation-shape guard creates real refactor friction; reuse the declarative engine rather than creating another one-off parser.
5. Behavior coverage for remaining facade splits and targeted performance/CSS work only when a real user-facing gap, measured regression, or safely reducible budget is demonstrated.
