...
- Typed Geometry / Build IR now owns both main-carcass cornice and shell geometry. `carcass_cornice_ir.ts` owns typed cornice segments, while `carcass_shell_ir.ts` owns stable board roles and back-panel geometry across normal, stepped, removed-side, and stack-split-prefixed identities. Producer and renderer share the canonical guards; renderer-local back-panel predicates and shell `MutableRecord` plumbing are removed, and malformed shell arrays fail closed before Three.js emission.
...
1. **Corner cornice plan-first IR — highest geometry value.** Extract a deterministic wing/connector plan from the corner path/profile family and validate it before Three.js emission.
2. **Canvas hover/paint preview contracts — highest interaction typing value.** Start with one explicit generic paint/hover preview envelope, then migrate sibling modes only if it removes duplicate `UnknownRecord` probing and `AppContainer` reach.
3. **Viewer Measurement capability DI expansion — targeted orchestration seam.** Narrow stable capability groups in the orchestrator without replacing `AppContainer` with a mega-deps bag.
4. **Planar reflector runtime — observe before decomposing.** Require a concrete ownership seam, recurring defect, or measured render cost; size alone is not debt.
5. **Contract Engine expansion on demand.** Reuse the declarative engine only when an active source/hash-heavy family proves real refactor friction.
6. Behavior coverage and targeted performance/CSS work only when a real user-facing gap, measured regression, or safely reducible budget is demonstrated.
