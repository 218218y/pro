...
1. **Contract Engine modernization.** Move high-value architecture contracts away from implementation-shape checks (source text, exact counts, function hashes) toward declarative AST ownership/import invariants plus behavior tests. Migrate one contract family at a time and keep source-shape checks only where no behavioral or structural invariant can express the requirement.
2. **TypeScript/control-plane simplification.** Promote the whole-project strict check to the primary type source of truth, then remove redundant per-layer typecheck modes/scripts only after proving they add no unique coverage.
3. **Capability DI pilot.** On one actively maintained feature boundary, replace broad `AppContainer`/runtime-access dependency reach with an explicit narrow capability object; expand only if the pilot measurably reduces imports, mocks, and accessors.
4. **Typed geometry/build IR.** Convert geometry descriptor families to discriminated typed plans when those areas are actively changed, enabling deterministic combinatorial invariant tests before Three.js emission.
