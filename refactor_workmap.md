...
1. **Canvas hover/paint preview contracts — highest interaction typing value.** Start with one explicit generic paint/hover preview envelope, then migrate sibling modes only if it removes duplicate `UnknownRecord` probing and `AppContainer` reach.
2. **Viewer Measurement capability DI expansion — targeted orchestration seam.** Narrow stable capability groups in the orchestrator without replacing `AppContainer` with a mega-deps bag.
3. **Planar reflector runtime — observe before decomposing.** Require a concrete ownership seam, recurring defect, or measured render cost; size alone is not debt.
4. **Contract Engine expansion on demand.** Reuse the declarative engine only when an active source/hash-heavy family proves real refactor friction.
5. Behavior coverage and targeted performance/CSS work only when a real user-facing gap, measured regression, or safely reducible budget is demonstrated.
