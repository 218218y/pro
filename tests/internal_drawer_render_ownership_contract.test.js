...
test('internal drawer runners consume the canonical cabinet runner selection', () => {
  const source = read(consumerRel);

  assert.match(source, /runnerType:\s*cfg\.drawerRunnerType/u);
  assert.doesNotMatch(source, /INTERNAL_DRAWER_RUNNER_TYPE/u);
});
