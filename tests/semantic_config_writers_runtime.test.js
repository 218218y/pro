...
test('[semantic-config-writers] semantic cfg access helpers prefer dedicated namespace methods and normalize scalar hotpaths', () => {
  /** @type {Array<Record<string, unknown>>} */
  const calls = [];
  const App = {
    actions: {
...
test('[semantic-config-writers] domain api stack/textures/doors writes mutate canonical config slices without legacy write paths', () => {
...
test('[semantic-config-writers] react store cfg wrappers keep named hotpaths on semantic helpers and canonical config writers', () => {
  /** @type {Array<Record<string, unknown>>} */
  const nsCalls = [];
  const nsApp = {
    actions: {
...
test('[semantic-config-writers] closed-union config writers reject invalid values without mutating config', () => {
  const { state, calls, store } = createStoreStub({
