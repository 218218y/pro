...
test('[react-write-wrappers] react UI wrappers prefer semantic UI namespace methods when installed', () => {
  /** @type {Array<Record<string, unknown>>} */
  const calls = [];
  const App = {
    actions: {
...
test('[react-write-wrappers] react UI/runtime wrappers use canonical slice writers with normalized patches', () => {
...
test('[react-write-wrappers] react UI/runtime wrappers fail fast without canonical actions instead of root store.patch fallback', () => {
  const state = createStoreState();
  /** @type {Array<Record<string, unknown>>} */
  const calls = [];
  const App = {
...
test('[react-write-wrappers] react config wrappers route through semantic config seams before generic scalar/map fallbacks', () => {
  /** @type {Array<Record<string, unknown>>} */
  const calls = [];
  const state = createStoreState();
