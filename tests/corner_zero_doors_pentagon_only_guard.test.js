...
test('[corner] 0 corner doors keeps only the pentagon connector and skips the wing', () => {
  ...
  assert.match(src, /const\s+cornerWingDoorCount\s*=\s*resolveCornerWingDoorCount\(/);
  assert.match(src, /if \(!\(cornerWingDoorCount > 0\)\) return;/);

  const earlyReturnPos = src.indexOf('if (!(cornerWingDoorCount > 0)) return;');
