...
test('[ui-build-reactivity] interactive ui commits update state once through root store.patch', () => {
...
test('[ui-build-reactivity] explicit no-build structural ui commits still patch root state only once', () => {
  const { state, calls, support } = createInteractiveUiHarness({
