...
test('build_state_resolver gates sketch-box door grooves behind groovesEnabled without mutating source config', () => {
    ...
    modulesConfiguration: [
      {
        ...
        sketchExtras: {
          boxes: [
            {
              id: 'sbf_alpha',
              doors: [{ id: 'sbdr_1', enabled: true, groove: true, grooveLinesCount: 5 }],
            },
          ],
        },
      },
    ...
    stackSplitLowerModulesConfiguration: [
      {
        sketchExtras: {
          boxes: [
            {
              id: 'sbf_lower',
              doors: [{ id: 'sbdr_2', enabled: true, groove: true, grooveLinesCount: 6 }],
            },
          ],
        },
      },
      ...
      modulesConfiguration: [
        {
          sketchExtras: {
            boxes: [
              {
                id: 'sbf_corner',
                doors: [{ id: 'sbdr_3', enabled: true, groove: true, grooveLinesCount: 7 }],
              },
            ],
          },
        },
        ...
        modulesConfiguration: [
          {
            sketchExtras: {
              boxes: [
                {
                  id: 'sbf_corner_lower',
                  doors: [{ id: 'sbdr_4', enabled: true, groove: true, grooveLinesCount: 8 }],
                },
              ],
            },
          },
  ...
  const baseState = {
    ui: { raw: { width: 160, height: 240, depth: 55, doors: 1 } },
    runtime: {},
    config: sourceConfig,
  };
  ...
  const disabledTopDoor = disabled.cfgSnapshot.modulesConfiguration[0].sketchExtras.boxes[0].doors[0];
  const disabledLowerDoor =
    disabled.cfgSnapshot.stackSplitLowerModulesConfiguration[0].sketchExtras.boxes[0].doors[0];
  const disabledCornerDoor =
    disabled.cfgSnapshot.cornerConfiguration.modulesConfiguration[0].sketchExtras.boxes[0].doors[0];
  const disabledCornerLowerDoor =
    disabled.cfgSnapshot.cornerConfiguration.stackSplitLower.modulesConfiguration[0].sketchExtras.boxes[0]
      .doors[0];

  assert.equal(disabledTopDoor.groove, false);
  assert.equal(disabledTopDoor.grooveLinesCount, null);
  assert.equal(disabledLowerDoor.groove, false);
  assert.equal(disabledLowerDoor.grooveLinesCount, null);
  assert.equal(disabledCornerDoor.groove, false);
  assert.equal(disabledCornerDoor.grooveLinesCount, null);
  assert.equal(disabledCornerLowerDoor.groove, false);
  assert.equal(disabledCornerLowerDoor.grooveLinesCount, null);

  assert.equal(sourceConfig.modulesConfiguration[0].sketchExtras.boxes[0].doors[0].groove, true);
  assert.equal(sourceConfig.modulesConfiguration[0].sketchExtras.boxes[0].doors[0].grooveLinesCount, 5);

  const enabled = resolveBuildStateOrThrow({
    App: {},
    stateOrOverride: {
      ...baseState,
      ui: { ...baseState.ui, groovesEnabled: true },
    },
  });

  const enabledTopDoor = enabled.cfgSnapshot.modulesConfiguration[0].sketchExtras.boxes[0].doors[0];
  assert.equal(enabledTopDoor.groove, true);
  assert.equal(enabledTopDoor.grooveLinesCount, 5);
});
