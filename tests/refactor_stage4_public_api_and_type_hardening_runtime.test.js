...
test('door authoring facade exposes only audited public symbols', () => {
  ...
  const contracts = [
    {
      reason:
        'build, corner, material refresh, and UI callers need the removal mode snapshot contract without importing the private owner',
      symbols: ['isRemoveDoorModeFromSnapshot', 'resolveRemoveDoorsEnabledFromSnapshots'],
    },
    {
      reason:
        'Design tab, paint apply, project normalization, and render builders share the door-style authoring value/token/map contract',
      symbols: [
    ...
    {
      reason:
        'Mirror authoring is shared by paint services, project maps, and render placement, so geometry/map helpers stay public through the feature facade',
      symbols: [
        'DEFAULT_FACE_SIGN',
  ...
  const expectedSymbols = contracts.flatMap(contract => contract.symbols).sort(compareCodePoints);

  for (const contract of contracts) {
    assert.ok(contract.reason.length > 40, `${contract.symbols.join(', ')} must explain public ownership`);
    assert.ok(contract.symbols.length > 0, `${contract.reason} must list symbols`);
  }
