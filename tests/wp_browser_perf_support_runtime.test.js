...
test('browser perf support summarizes runtime issues and perf metrics canonically', () => {
  const result = {
    ...
    windowBuildDebugSummary: {
      ...
      immediateNonForceRequestCount: 2,
      debouncedForceRequestCount: 1,
      debouncedNonForceRequestCount: 3,
      ...
      executeDebouncedNonForceCount: 2,
    ...
    windowBuildDebugTopReasons: [
      {
        ...
        debouncedForceRequestCount: 1,
        debouncedNonForceRequestCount: 2,
        executeImmediateCount: 1,
    ...
    journeyBuildPressureSummary: {
      'cabinet-build-variants': {
        ...
        immediateNonForceRequestCount: 2,
        debouncedForceRequestCount: 1,
        debouncedNonForceRequestCount: 6,
        executeImmediateCount: 1,
        ...
        executeImmediateNonForceCount: 1,
        executeDebouncedForceCount: 1,
        executeDebouncedNonForceCount: 4,
  ...
  assert.match(
    md,
    /project\.save-load\.roundtrip: requests=6, executes=4, immediateForce=0, immediateNonForce=2, coalescedForce=1, coalescedNonForce=3, forceRequests=1, forceExecutes=1, pendingOverwrites=2, suppressedRequests=1, debounce=4, duration=440ms, topReasons=apply-board-material/
  );
  assert.match(md, /Top build reasons/);
  assert.match(
    md,
    /apply-board-material: requests=4, executes=3, immediateRequests=1, debouncedRequests=3, immediateForce=0, immediateNonForce=1, coalescedForce=1, coalescedNonForce=2, forceRequests=1, forceExecutes=1/
  );
  assert.match(md, /Build-heavy customer journeys/);
  assert.match(
    md,
    /cabinet-build-variants: steps=2, requests=9, executes=6, immediateForce=0, immediateNonForce=2, coalescedForce=1, coalescedNonForce=6, forceRequests=1, forceExecutes=1, pendingOverwrites=3, suppressedRequests=2, debounce=5, total=780ms, topReasons=apply-board-material, toggle-door-style/
  );
...
test('browser perf support summarizes build scheduling pressure canonically', () => {
  const stats = {
    ...
    executeForceCount: 1,
    executeImmediateForceCount: 0,
    executeImmediateNonForceCount: 2,
    executeDebouncedForceCount: 1,
    executeDebouncedNonForceCount: 4,
    pendingOverwriteCount: 3,
    ...
    reasons: {
      'apply-board-material': {
        ...
        immediateNonForceRequestCount: 1,
        debouncedForceRequestCount: 1,
        debouncedNonForceRequestCount: 4,
        executeImmediateCount: 1,
        executeDebouncedCount: 3,
        executeForceCount: 1,
        executeImmediateForceCount: 0,
  ...
  assert.deepEqual(flowSummary['cabinet-build-variants.profile-texture.configure'], {
    ...
    immediateNonForceRequestCount: 1,
    debouncedForceRequestCount: 1,
    debouncedNonForceRequestCount: 5,
    executeImmediateCount: 1,
    executeDebouncedCount: 5,
    executeForceCount: 1,
    executeImmediateForceCount: 0,
  ...
  const journeySummary = createJourneyBuildPressureSummary([
    ...
    },
    {
      ...
      after: {
        ...
        forceRequestCount: 3,
        immediateForceRequestCount: 1,
        immediateNonForceRequestCount: 3,
        debouncedForceRequestCount: 2,
        debouncedNonForceRequestCount: 8,
        executeCount: 10,
        executeImmediateCount: 2,
        ...
        executeImmediateForceCount: 0,
        executeImmediateNonForceCount: 2,
        executeDebouncedForceCount: 2,
        executeDebouncedNonForceCount: 6,
        ...
        reasons: {
          'apply-board-material': {
            ...
            executeImmediateNonForceCount: 1,
            executeDebouncedForceCount: 2,
            executeDebouncedNonForceCount: 3,
          },
          'toggle-door-style': {
            requestCount: 6,
            executeCount: 4,
  ...
  assert.deepEqual(journeySummary['cabinet-build-variants'], {
    ...
    immediateNonForceRequestCount: 2,
    debouncedForceRequestCount: 2,
    debouncedNonForceRequestCount: 7,
    executeImmediateCount: 1,
    executeDebouncedCount: 8,
    executeForceCount: 2,
    executeImmediateForceCount: 0,
...
test('browser perf support explains Long Tasks with exact builder, render, and slow-store intervals', () => {
  const rows = createLongTaskRootCauseSummary({
    ...
    windowPerfEntries: [
      ...
      perfEntry('mirror.direct-refresh', 10, 'ok', {
        kind: 'phase',
        browserSessionId: 'doc-1',
        startTime: 260,
        endTime: 270,
  ...
  assert.deepEqual(rows[0].renderPhaseContributionsMs, { mirror: 10, motion: 5, renderer: 72 });
  ...
  assert.equal(rows[0].unattributedMs, 10);
});
