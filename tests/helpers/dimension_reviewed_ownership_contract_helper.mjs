...
function graphStatementKey(statement) {
  return JSON.stringify({
    from: statement.from,
    to: statement.to,
...
export function assertReviewedOwnershipFamily({ root, label, entryNumbers }) {
  ...
  const graphByExactStatement = new Map();
  for (const statement of graph.imports) {
    const key = graphStatementKey(statement);
    const matches = graphByExactStatement.get(key) ?? [];
    matches.push(statement);
    graphByExactStatement.set(key, matches);
  }
