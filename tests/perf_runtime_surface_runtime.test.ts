...
test('perf renderer program owner probe maps an existing program without creating renderer properties', () => {
  const targetProgram = { id: 77, cacheKey: 'diagnostic-owner-program', usedTimes: 2 };
  const targetMaterial = {
    id: 11,
    uuid: 'material-owner-uuid',
    type: 'MeshStandardMaterial',
    name: 'door-trim-nickel',
    userData: {
      __wpPerfPersistentCacheOwner: 'door-trim',
      isCached: false,
      semanticRole: 'trim',
      nested: { ignored: true },
    },
  };
  const untouchedMaterial = { id: 12, uuid: 'untouched-material' };
  const materialProperties = new WeakMap<object, Record<string, unknown>>();
  materialProperties.set(targetMaterial, {
    programs: new Map([['diagnostic-owner-program', targetProgram]]),
  });
  let propertyGetCount = 0;
  const app = {
    render: {
      renderer: {
        info: { programs: [targetProgram] },
        properties: {
          has(value: object) {
            return materialProperties.has(value);
          },
          get(value: object) {
            propertyGetCount += 1;
            return materialProperties.get(value);
          },
        },
      },
      cache: {
        materialCache: new Map([['canonical-target', targetMaterial]]),
      },
      scene: {
        id: 1,
        uuid: 'scene-uuid',
        ...
        children: [
          {
            id: 21,
            uuid: 'mesh-owner-uuid',
            type: 'Mesh',
            name: 'door-trim-mesh',
            userData: { __kind: 'door_trim', partId: 'door-1', nested: { ignored: true } },
            material: targetMaterial,
            children: [],
          },
          {
            id: 22,
            uuid: 'mesh-untouched-uuid',
            type: 'Mesh',
            material: untouchedMaterial,
            children: [],
          },
        ],
      },
  ...
  const surface = createPerfConsoleSurface(app as never);
  const targetKey = surface.getRendererProgramSnapshot?.()?.programs[0]?.key;
  assert.ok(targetKey?.startsWith('cache:'));
  assert.equal(typeof surface.getRendererProgramOwnerSnapshot, 'function');
  const snapshot = surface.getRendererProgramOwnerSnapshot?.([targetKey as string]);

  assert.equal(snapshot?.rendererPropertiesSupported, true);
  assert.equal(snapshot?.traversedObjectCount, 3);
  assert.equal(snapshot?.matches.length, 1);
  assert.equal(propertyGetCount, 1, 'properties.get() must only run after properties.has() succeeds');
  assert.deepEqual(snapshot?.matches[0], {
    programKey: targetKey,
    programId: 77,
    programUsedTimes: 2,
    programCacheKeyHash: targetKey?.slice('cache:'.length),
    rawProgramCacheKey: 'diagnostic-owner-program',
    materialDiagnosticId: snapshot?.matches[0]?.materialDiagnosticId,
    materialId: 11,
    materialUuid: 'material-owner-uuid',
    materialType: 'MeshStandardMaterial',
    materialName: 'door-trim-nickel',
    isCached: false,
    persistentCacheOwner: 'door-trim',
    canonicalMaterialCacheKeys: ['canonical-target'],
    materialUserData: {
      __wpPerfPersistentCacheOwner: 'door-trim',
      isCached: false,
      semanticRole: 'trim',
    ...
    sceneObjects: [
      {
        objectId: 21,
        objectUuid: 'mesh-owner-uuid',
        objectType: 'Mesh',
        objectName: 'door-trim-mesh',
        materialIndex: 0,
        userData: { __kind: 'door_trim', partId: 'door-1' },
      },
    ],
  });
});
