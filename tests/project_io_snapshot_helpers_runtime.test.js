...
test('[project-io-snapshots] project load helpers shape canonical config/ui snapshots instead of depending on owner-file text layout', () => {
...
test('[project-io-snapshots] project load helpers preserve runtime UI ephemera and capture source flags semantically', () => {
  const result = runTsModule(`(() => ({
...
test('[project-io-snapshots] save helpers normalize persisted payloads and default project snapshots by data meaning, not file structure', () => {
  const result = runTsModule(`(() => {
