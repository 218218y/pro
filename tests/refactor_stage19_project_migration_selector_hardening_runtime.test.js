...
test('stage 19 project canonical selector hardening runtime tests pass', () => {
  runNode([
...
test('stage 19 project canonical selector guard is wired into the refactor control plane', () => {
...
test('stage 33 to 35 project config canonical replace-owned branches are anchored', () => {
  const projectLoad = readFileSync('esm/native/io/project_io_orchestrator_project_load.ts', 'utf8');
  const canonicalSnapshot = readFileSync('esm/native/io/project_load_canonical_snapshot.ts', 'utf8');
  ...
  assert.match(canonicalSnapshot, /PROJECT_CONFIG_SNAPSHOT_REPLACE_KEYS/);
  assert.match(canonicalSnapshot, /PROJECT_CONFIG_SNAPSHOT_REQUIRED_KEYS/);
  ...
  assertCatalogAnchorsNeedle('stage 33 to 35 project config canonical replace-owned branches are anchored');
...
test('stage 36 to 38 deterministic project config replace-key closeout is anchored', () => {
  ...
  for (const symbol of [
    ...
    assert.match(canonicalSnapshot, new RegExp(`\\b${symbol}\\b`));
    assert.match(boundaryAudit, new RegExp(`\\b${symbol}\\b`));
  }
  assert.match(canonicalSnapshot, /buildProjectConfigSnapshotReplaceKeyMap/);
  assert.doesNotMatch(
    canonicalSnapshot,
