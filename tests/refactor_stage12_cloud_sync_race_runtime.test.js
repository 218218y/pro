...
test('stage 12 cloud sync race guard is wired into refactor guardrails', () => {
  ...
  assert.match(pkg.scripts['test:refactor-stage-guards'], /refactor_stage12_cloud_sync_race_runtime\.test\.js/);
});
...
test('stage 12 cloud sync race fix resets stale in-flight follow-up queues', () => {
  ...
  assert.match(source, /context\.state\.queued && \(context\.deps\.isDisposed\(\) \|\| context\.deps\.isSuppressed\(\)\)/);
